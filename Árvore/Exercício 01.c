//Escreva uma função que conta o número total de nós de uma Árvore Binária de Busca.
...
int  quantidadeDeNo(ARVORE arv){
	return totalNo(arv.raiz);
}
...
void main() {
	...
	inserirNaArvore(&arv, 34);
	inserirNaArvore(&arv, 20);
	imprimir(arv);
	printf("\n");

	printf("\nA quantidade de nos eh : %d ",	quantidadeDeNo(arv));
}
